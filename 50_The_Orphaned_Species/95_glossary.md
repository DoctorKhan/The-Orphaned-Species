...
> **Canon note on the god-factions.** Two generations, **each weaker than its maker** (the ladder serpents → elder gods → younger gods → humans). The **elder, creating generation = the Anunnaki (Mesopotamian) = the Titans (Greek)** — Enki's faction: the golden-age gods who install the **locks**, argue for human freedom, and **crash** in Phase V. The **younger, controlling generation = the Igigi (Mesopotamian) = the Olympians (Greek)** — Enlil's faction: *weaker*, who sabotage their elders (the **Titanomachy / Generational War**) and run the cage. *(This refines the older note: the prior* Human Experiment *draft used "Titans" for the elder and "Anunnaki" for the younger; Book 5 fixes elder = Anunnaki/Titans and younger = Igigi/Olympians — so "Titans" returns, now correctly as the **Greek name for the elder Anunnaki**, not a separate group.)* See `00_MASTER_TIMELINE.md`.
>
...
**The Two Trees (as cost)** — **Tree of Knowledge** = encoded communication (speech, symbol); **Tree of Life** = direct connection (telepathy, resonance). Each costs the same metabolism; **both is cumulative.** So **humans** (Knowledge, one-Tree) ≈ **dragons & aliens** (Life, one-Tree) < **gods / Titans** (both, two-Tree — the anomaly, the only ones needing extraction).
