...
**Trick:** the crossing goes *under*, on the elder's breath discipline and route knowledge — preparation, not panic. Past a certain count the diaphragm slams into contractions that feel like drowning starting early; that spasm is the body's carbon-dioxide alarm, not real failure, and fighting it only burns the air that would have carried the diver through (real mechanism: chemoreceptor CO2 response, not oxygen debt). Timah does not teach Eli to want it less. She teaches him to stop arguing with it — let the heart rate drop, let the shift happen — so the wall he hit as catastrophe in Malacca turns out to have a door in it, one villagers cross so often the local kids' name for the game is *going dolphin* and Timah's own word for the far side is closer to *arriving* than to power. The same asphyxia threshold that broke Eli open as catastrophe (seed 3) is her ordinary workday, held open safely by inheritance and practice — and it is the same fragment-of-control drill he already has for psi overload (`15` § First test: one counted exhale, attention to sole of foot), practiced at a different depth. Her teaching is the book's answer to the breach, delivered by someone who has no idea cosmology exists.
