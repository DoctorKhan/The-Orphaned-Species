...
**Evidence.** `99_open_questions.md` #12 (native-road mechanism) is open with the right sub-questions: how many people to hold the proof? what can instruments verify without owning carriers? what stops Crane re-monopolizing it next morning? what stays *hard* so it isn't a switch-flip?
...
**Evidence.** `27_character_development_arcs.md` belief→behavior table (every row resolves in a hard no); scene seed "refuses prophecy language once per volume" (`26`); the Vol III double-temptation (`33` Ch 27).

**Suggested direction.** Two correctives:
- **Make some hard nos genuinely wrong.** The Ch 26 seed (his restraint gets a woman killed, `49` Scene 0) is exactly this — do more of it. A hard no that costs an innocent is the only thing that makes the *next* limit suspenseful.
