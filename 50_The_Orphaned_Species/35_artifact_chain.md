# The Artifact and Story Chain

*Working plot document. Artifacts carry contested stories through the present-day thriller. They are evidence, not collectibles, omniscient narrators, or automatic portals into historical chapters.*

## Governing rule

The present plot can recover the past through **objects and stories without repeatedly going back in time**. An artifact may produce:

- a scholarly translation;
- a local or inherited story;
- a faction's strategic interpretation;
- forensic evidence about material, age, route, or alteration;
- a short resonance impression that supplies sensory texture but not objective explanation.

Those accounts should disagree. The characters remain in the present, test the versions against one another, and act on incomplete conclusions. A full historical descent is optional and must earn its place through dramatic necessity; finding an artifact never triggers one automatically.

Artifacts are not a quest inventory. Each object must change the route, divide the group, expose a false interpretation, or force a costly choice. Some should be authentic ancient objects, some later copies, some missing fragments known only from scans or stories, and at least one a planted or misidentified object. The protagonists should rarely possess a famous museum object; photographs, squeezes, casts, translations, private fragments, stolen metadata, and displaced custodians are usually more plausible.

## Artifact zero — the Double-Fork Stone

The chain's oldest and most dangerous object is a small carved tablet, seal, bone, or stone showing **two Y-shaped divergences**:

1. an older reptilian/archosaur root dividing into **bird** and **dragon**;
2. an older primate root dividing into **chimpanzee** and **human**.

The image represents sibling branches from common roots. It must never imply that birds descend from dragons or humans descend from chimpanzees.

### Visual grammar

- The upper fork begins with a scaled or egg-like root. One arm resolves into feather, wing, or bird track; the other into coiled body, thermal mark, cave, or field-wave.
- The lower fork begins with a primate hand or 48-mark count. One arm resolves into knuckle/forest signs; the other into an open hand with the chromosome-fusion seam or receiver mark.
- Neither fork points upward. A horizontal or mirrored layout prevents the image from becoming a ladder of superiority.
- The human and dragon branches may share a signal mark while meaning different things: the dragon is a native broadcaster; the human is a modified receiver. The similarity is functional, not genealogical.

### Plot logic

The characters initially mistake the carving for a divine genealogy, heraldic emblem, or later forgery. The human–chimp fork becomes the reading key because independent genetics supports common ancestry while chromosome counts and the human chromosome-2 fusion distinguish the branches. That partial confirmation does **not** prove the dragon branch. It upgrades the upper fork from fantasy to a testable hypothesis and sends the group looking for biological or archaeological corroboration.

The artifact tells a story central to the coexistence test: humanity is one branch among branches, not the summit of creation. Birds, chimpanzees, dragons, and humans represent different survivals and relationships to the field. Difference does not establish a right to rule or exterminate.

### Guardrails

- The Double-Fork Stone is fictional unless a real archaeological analogue is later documented. Never market it as a recovered real-world artifact.
- Its dating, find context, and chain of custody must be disputed.
- It cannot contain a complete evolutionary tree or modern scientific labels.
- The group needs at least one wrong reading and one independent material test.
- It should create the next investigation, not settle the cosmology.

## Candidate chain

| Artifact or story-object | Historical status | Story it carries | Present plot function |
|---|---|---|---|
| **Double-Fork Stone** | Fictional story artifact | Dragon/bird and human/chimpanzee as sibling divergences; humanity is not the apex | Establishes the method: validate the readable lower fork, then investigate the impossible upper fork |
| **Sumerian King List / Weld-Blundell tradition** | Real textual tradition and surviving manuscripts | Kingship descends from heaven, moves between cities, breaks at the Flood, then descends again | Reveals that the control memory concerns an **office**, not a sacred bloodline; points toward Eridu and the reset story |
| **Adapa tablet fragment** | Real textual tradition, fragmentary witnesses | A human refuses life after receiving a warning he cannot verify | Establishes wrong knowledge as a control technology and makes trust in a helpful god materially ambiguous |
| **Asherah amulet, pendant, or inscribed shard** | Exact object TBD; use a fictional object only if clearly labeled, grounded against real Asherah-associated inscriptions and iconography | A female divine/tree relation survives underneath later father-only theology | Exposes deliberate erasure and gives Wren or another custodian an interpretation independent of Eli and Emrys |
| **Atrahasis / Eridu Flood fragment** | Real textual traditions | Divine labor, worker humanity, catastrophe, and reset survive in incompatible versions | Reveals that creation stories can be labor propaganda layered over older events |
| **Tablet of Destinies fragments** | Mythic object; any excavated physical version in the novel is fictional or a ritual/administrative copy | Authority is imagined as possession of the right to decree, route, name, and command | Becomes evidence about grid authorization, not an all-powerful magical remote; factions disagree over whether it is key, charter, map, or propaganda |
| **Ugaritic divine-council tablet** | Real textual corpus; exact tablet to select and verify | El, Athirat/Asherah, Baal, Anat, and the divine assembly preserve offices later collapsed or renamed | Lets the group compare pantheons as political memories without claiming every god-name is literally identical |
| **Babylonian Map of the World** | Real artifact/textual map | The known world sits inside a larger mythic geography whose edges are narrated rather than measured | Supplies a deliberately incomplete topology; becomes useful only when compared with changing ley-line data |
| **Nebra Sky Disc or another verified European sky-object** | Real artifact; use requires route/date research | Sky knowledge can be encoded in portable material without becoming a universal key | Bridges the Near Eastern story chain toward Europe while forcing the group to distinguish transmitted knowledge from independent discovery |
| **Cuno–Derw marked object** | Fictional artifact already native to the story | Geometry, rhythm, body positions, and a lossy account survive the last firing | Converges the artifact stories in Albion without providing a complete manual or proving one bloodline owns the knowledge |

## The Asherah and Destinies guardrails

The **Asherah object** and **Tablet of Destinies** are valuable precisely because their statuses differ from the King List. The King List is a real textual tradition. The Tablet of Destinies is a mythic object inside ancient stories. An exact "Amulet of Asherah" must either be tied to a verified archaeological object or acknowledged as fiction. The novel should make those evidentiary levels visible rather than flattening all three into equally authenticated relics.

The Tablet of Destinies should be distributed across fragments, copies, translations, and claims. If one intact object can command the grid, the story collapses into a treasure hunt. If several partial objects preserve competing accounts of who had authority to route the network, the artifact becomes evidence about civilization—the office everyone wants to inherit.

## Story-delivery rule

For every artifact, use no more than the story the current decision requires. A translation scene can yield one disputed sentence. A custodian can tell a five-minute legend while hiding where the object came from. A resonance event can supply heat, fear, a bodily position, or one image without identifying the speaker. Later evidence may reverse the meaning.

The governing sequence is:

> object → competing stories → test → decision → consequence

Never:

> object → certified flashback → complete answer
