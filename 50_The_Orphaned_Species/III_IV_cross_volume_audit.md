# Book III ↔ Book IV Cross-Volume Consistency Audit
*Generated from on-disk drafts and architecture locks.*

> **Rebuilt 2026-08-10.** The prior version of this audit was generated against `34_volume_IV_beatsheet.md`, whose entire ten-chapter spine was superseded 2026-08-07–09. The governing Book IV architecture is now `60_volume_IV_court_of_threads.md`; this rebuild checks against that file and the current Book III endgame (`33_volume_III_beatsheet.md` § *BOOK III ENDGAME*, revised 2026-08-07) instead.

## Summary
Both manuscripts are internally consistent with their own current architecture docs and cross-consistent with each other on all load-bearing beats. The Costa Rica place-name gap flagged in the prior audit is resolved — the names are now drafted into the manuscript. No open structural gaps remain; two research-consultation items remain (unchanged, not story-blocking).

## Cross-volume hinge

| Lock | Book III exit | Book IV opening | Status |
|---|---|---|---|
| Decentralized settlement holds, but untested at scale | Handover succeeds locally; distributed credentials; no single point of failure *in the room* — Continuity keys/factional mandate/private force outside it stay live and unresolved (`33` Ch 10 card) | Seven months of the system working exactly as designed, then Ch 1 opens on the first missing delegate in seven months — the manuscript dramatizes the untested claim being tested for real, not just restating that it held (`IV` Ch 1, "something is finally testing whether it still holds") | Consistent — and correctly plays the "proven for the person, untested for the system" doctrine, not the older "clean win" framing |
| Continuity clause | Dormant; read into record; Wren ensures "not currently in effect" on tape | Institutions deliberately invoke it; Henley's compromised credentials; extension request during labor | Consistent |
| Eli's standing | Sits among ordinary delegates; Rowan/Wren present but separate | Same; Eli still among delegates; no platform | Consistent |
| Rowan/Eli relationship | Love acknowledged while Eli commands; no sex or domestic access before handover; coda kiss permitted | Established love consummated; family and conception decisions remain separate | Consistent (`19` and `60` § 5 *Romance and family architecture*) |
| Wren/Rowan | Both intact; no reconciliation scene needed | Both intact; Rowan negotiates with Wren before acting | Consistent |
| Amara | Unforgiving; "a process can be real without making me feel better" | Still unforgiving; appears in court, does not forgive Eli | Consistent |
| Cloisters | Founded by Extra men, veterans, Thread elders, Eli; Owen/Malik leadership | Formalized; praetorian chapters split; civilian defense oath; Owen/Malik still present | Consistent |
| Weavers | Wren, Rowan, Asha build living attestations | Weaver monopoly ends; attestations released to review body; houses retain seats, lose custody standing | Consistent |

## Mandated Book IV beats — manuscript coverage (checked against `60` § 9's ten-chapter spine)

| Beat | Manuscript location | Status |
|---|---|---|
| Seven-month successful stretch, then first failure | Ch 1 opening | Present |
| The Hollow Year eruption + cross-network seizure | Ch 1–2 pressure | Present |
| Costa Rica evacuation hinge (Nosara–Sámara, Tenorio–Miravalles) | Ch 5 — place names drafted (line ~2679) | **Present — resolved since prior audit** |
| Continuity clause invoked by institutions | Ch 2–9 (Henley credentials, extension request, final motion) | Present |
| Rowan/Eli love → post-handover consummation → family decision → conception | Book III acknowledgement; Book IV Ch 2 sex; Ch 4 family decision; Ch 6 pregnancy | Present |
| Wren boundary / triad refusal | Ch 3 (Wren/Rowan corridor conversation) | Present |
| Sports/market gathering dispute | Ch 7 | Present |
| False heir / dynasty motion | Ch 8 | Present |
| Birth simultaneity with transfer | Ch 9–10 (transfer at 16:43; birth at 17:06) | Present, verified in manuscript text |
| Tara born without title | Ch 10 | Present |
| Teahouse/garden coda | Ch 10 closing | Present |
| Swifts morning rhyme | Ch 10 closing | Present |

## Deep-time residue check

| Volume | Quota | Status |
|---|---|---|
| Book III | ~1 every 1–2 chapters | Confirmed in manuscript |
| Book IV | ~1 every 1–2 chapters | **Checked by hand 2026-08-10.** Ch 7 and Ch 9 already carry strong residue (the roof-chant/old-practice beat in Ch 7; the chalked circle and twelve-chair litany in Ch 9) — the earlier "missing" flag was a script artifact (keyword regex didn't match this vocabulary), not a real gap. Ch 3 and Ch 8 genuinely lacked Book IV's specific mandated register ("court as fossil temple — offices without gods") and got one short passage each: Wren on the audience-category directive as a degraded palace registry (Ch 3), Rowan on the Continuity Household Settlement as a throne's absence with the temple's shape kept (Ch 8). |

## POV rules

| Volume | Lock | Manuscript |
|---|---|---|
| Book III | Eli home; Wren owns one chapter | Consistent |
| Book IV | Wren/Rowan principal, alternating by chapter; Eli from outside, no POV chapter | Consistent with `60` § 9 chapter-by-chapter POV assignments |

## Open items

1. Research consultations still required before final prose for Hawaiʻi/Pele and Brazilian municipality selection (unchanged, not story-blocking).

## Conclusion
The manuscripts are written, internally consistent with `60_volume_IV_court_of_threads.md` (the current governing Book IV architecture, not the retired `34`), and cross-volume consistent with Book III's current endgame. The Costa Rica place-name gap is closed. No structural rewrite required.
