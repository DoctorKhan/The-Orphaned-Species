...
**Follow-up sweep — COMPLETE (2026-07 four-term pass + this pass).** Legacy conceptual terms (*memory capsule, lived-life/watched-vision, Aedan relay, resonance braid*) now survive only in intentional deprecation lists (`36` §2, `37` §5/§12, `95`, this file); the `36` §10 four-term sweep (protocol/shard/mesh/node) is done — *shard* and *node* are now canonical web-vocabulary, not legacy jargon. Live-frame leftovers fixed: `10` (carrier-imprints→practitioner imprints), `15` (carrier-memory→deep-thread memory; historical carrier→practitioner), `25`/`39` (protocol→practice). `36` governs.
...
**Priority 1 is locked (#1–#6) and the Volume I beat sheet exists: `31_volume_I_beatsheet.md`** (14 chapters, no prologue; chapter-card format). **Trilogy plot hub:** `29_trilogy_plot_and_development.md` — act-level Vol II–III, revelation links, open gaps (#7–#14).
